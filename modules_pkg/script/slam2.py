...
def jacobian_g_state_linear(state, action, delta):
    # state: [x, y, theta]
    # action: [v, w]
    x, y, theta = state
    v, w = action
    return np.array([[1, 0, -v *delta * np.sin(theta)],\
...
def jacobian_g_state(state, action, delta):
    ...
    v, w = action
    if w == 0:
        return jacobian_g_state_linear(state, action, delta)
    #
    return np.array([[1, 0, v/w * (-np.cos(theta) +np.cos(theta + w *delta))],\
            [0, 1, v/w * (-np.sin(theta) +np.sin(theta + w *delta))],\
            [0, 0, 1]])

# jacobian with respect to action
def jacobian_g_action_linear(state, action, delta):
    # state: [x, y, theta]
    # action: [v, w]
    x, y, theta = state
    v, w = action
    #
    return np.array([[delta *np.cos(theta), 0],\
            [delta *np.sin(theta), 0],\
            [0, 1]])

def jacobian_g_action(state, action, dt):
    # state: [x, y, th]
    # action: [v, w]
    x, y, th = state
    v, w = action
    if w == 0:
        return jacobian_g_action_linear(state, action, dt)
    #
    return np.array([\
            [(np.sin(dt*w +th) - np.sin(th))/w, v*(np.sin(th) -np.sin(dt*w +th)+dt*w*np.cos(dt*w+th))/w**2],\
            [(np.cos(th)-np.cos(dt*w +th))/w, v*(-np.cos(th) +np.cos(dt*w +th)+dt*w*np.sin(dt*w+th))/w**2],\
            [0, dt]])
...
def slam_callback(sensors_msg):
    ...
    # motion cov 6x6 matrix
    motion_cov = odom.twist.covariance
    ...
    G = jacobian_g_state(state, action, delta)
    V = jacobian_g_action(state, action, delta)
    state_cov = G @ state_cov @ G.T
    state_cov = state_cov + V @ np.diag([.1, .1]) @ V.T
